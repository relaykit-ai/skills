...
<!-- gen:skill-send:end -->
   ...
   `data` carries every variable the message names — the catalog's `variables` array for that message, nothing more and nothing less. `tone` is optional: `standard`, `friendly` or `brief`; leave it out for `standard`.
...
5. **Verify with a phone, not a status code.** A test key (`rk_test_…`) only reaches numbers that have verified as testers: `POST /v1/sandbox/recipients` with `{ "phone": "+1…" }`, RelayKit texts that phone a code, and the person replies YES (or enters the code). Send to a verified tester, then ask the person whether their phone buzzed. That is the test. A 200 is not.
...
- **Retries carry an `Idempotency-Key` header** — any string, unique per logical send. The same key inside 24 hours replays the first result instead of sending twice.
